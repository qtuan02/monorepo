...
- [`apps/chat`](./apps/chat/CONTEXT.md) — app nhắn tin real-time (Runtime Vite, port từ `chat-socket-fe`, backend `chat-socket` REST + STOMP): Session (token in-memory + refresh cookie), Health gate, Presence, Conversation (direct/group), Draft conversation, Friend request. Spec #195; glossary ghi trước khi `gen:app` chạy (grill 2026-09-18); thư mục chưa có `package.json` cho tới ticket đầu.
