...
export function useMessageComposer(
  ...
  const isSendDisabled = isPending || content.trim() === "";
  ...
  const handleSubmit = React.useCallback(async () => {
    ...
    // TEXT only: the attachment half of T2 (spec #253) is parked until the
    // backend serves uploads — the receive side (message-row.tsx) still
    // renders IMAGE/FILE from other clients.
    setContent("");
    try {
