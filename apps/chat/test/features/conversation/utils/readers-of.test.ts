...
describe("isSeenByOther", () => {
  ...
  it("is true once their lastReadAt reaches this message's own createdAt", () => {
    const other = member({
      userId: "u2",
      lastReadAt: "2026-09-19T08:00:00.000Z",
    });

    expect(
      isSeenByOther(message("m1", "2026-09-19T08:00:00.000Z"), other),
    ).toBe(true);
  });
  ...
  it("stays true when they've read past this message (a later reply of their own)", () => {
    const other = member({
      userId: "u2",
      lastReadAt: "2026-09-19T09:00:00.000Z",
    });
  ...
  it("is false while their lastReadAt is still before this message", () => {
    const other = member({
      userId: "u2",
      lastReadAt: "2026-09-19T07:00:00.000Z",
    });
