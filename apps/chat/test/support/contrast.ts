 ...
 * lightness axis (which is not what WCAG measures). This app's own override
 * is written in `oklch()` too (the source app's own values), so
 * `test/globals.test.ts` uses the OKLCH half directly rather than
 * `hexToRgb` — the latter stays here only because it is the same file
 * `apps/portfolio`/`apps/smart-rental` carry.
 *
...
/** Euclidean distance in 8-bit sRGB — `0` is the same colour, `~441` is black↔white. */
