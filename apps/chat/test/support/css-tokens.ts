 ...
 * properties that only a real cascade resolves. What a unit test can assert
 * is the contract a stylesheet writes — which tokens, in which block, with
 * which value — and these helpers read that contract structurally rather
 * than by position. `test/globals.test.ts` checks the override against the
 * shared theme with them.
 */
...
export function atRuleRegions(source: string, name = "[\\w-]+"): string[] {
  ...
  for (const match of source.matchAll(opener)) {
    // Skip an at-rule nested inside one already taken, so a rule inside
...
/**
 * `source` with every at-rule block removed — what is left is the
 * declarations that apply unlayered and unconditionally. This is the
 * distinction the whole override rests on, so it is read structurally
 ...
 * makes every lookup `string | undefined`, and a silent `?? ""` fallback
 * would turn a token that has been renamed away into a passing assertion.
 */
