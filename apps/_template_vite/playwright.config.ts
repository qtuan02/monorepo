...
export default defineConfig({
  ...
  webServer: {
    // `--strictPort` so the preview server *this config starts* fails loudly
    // instead of drifting to 3001 and leaving Playwright to wait out its
    // 180s timeout on a port nothing is listening to.
    //
    // It is NOT a guard against a leftover dev server on 3000: Playwright
    // probes `url` before it ever runs `command`, so anything already
    // answering there is reused locally (and rejected outright in CI) and this
    // line never executes. Kill whatever holds 3000 yourself before an e2e
    // run — a stale dev server serves stale modules to every spec and the run
    // still looks green.
    command: "bun run build && bun run preview --port 3000 --strictPort",
    url: BASE_URL,
