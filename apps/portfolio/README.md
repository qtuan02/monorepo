...
nguồn, Website, giá trị liên hệ) có vùng bấm 32px. Từ tablet dọc (`md`) trang là **cột đọc** +
**rail** 3/2, hai cột 2/1 giữ nguyên từ `lg` — rail cuộn cùng trang chứ không dính (#270: rail cao
hơn khung nhìn laptop, dính sẽ ghim Liên hệ/Sở thích ra ngoài màn); landscape phone giữ lề gốc, `md`+
mới lên `pt-24`. Cam kết không tràn ngang từ 320px. `e2e/viewport.e2e.ts` và `e2e/dock.e2e.ts` đo cả bộ.
...
| Thứ | Ở đâu | Ghi chú |
| --- | --- | --- |
| Bố cục | `src/app/[locale]/(shell)/layout.tsx` + `templates/home.template.tsx` | Khung `max-w-6xl`. Một cột dưới `md`; từ `md` là **cột đọc** (About, Work, Projects) + **rail** (Skills, Education, Contact, Hobbies), hero trải cả hai — tỉ lệ 3/2 ở tablet dọc (768px, rail ~264px), 2/1 từ `lg` (#123, #210). Rail cuộn cùng trang thay vì dính (#270): cao hơn khung nhìn một laptop, nên dính sẽ ghim Liên hệ/Sở thích ra ngoài màn tới khi cột đọc cuộn hết. Đúng thứ tự DOM cắt đôi, nên test thứ tự section không đổi và tab order = thứ tự đọc. Projects là một cột — hai khối xếp dọc, không còn lưới nhiều cột (#269); trong rail Skills xếp nhãn trên giá trị từ `md`, Contact/Hobbies xuống dọc từ `md` (giữ hai cột ở `sm`). Landscape phone (667×375) giữ lề `py-12`, `md`+ mới lên `pt-24`; `scroll-padding-bottom` trên `html` chừa chỗ cho thanh chạm đáy khi focus nhảy tới link cuối. `e2e/viewport.e2e.ts` đo cả bốn bố cục, cam kết không tràn ngang từ 320px. |
...
| Hero | `src/features/home/components/hero-section.tsx` | **Cửa sổ terminal duy nhất** của trang: một `StandardBlock` với `p-0` mang thanh tiêu đề (ba chấm `rounded-full` — **ngoại lệ bo tròn cố ý duy nhất** của trang, vì đèn cửa sổ thì tròn — + `tuan@portfolio:~`, `aria-hidden`, `print:hidden`) rồi thân theo thứ tự một shell in ra — `$ whoami` → tên (**h1** — không còn lời chào, không emoji), `$ cat role.txt` → **chức danh trên mảng vàng** (`bg-highlight`, `inline-block` nên vệt vàng ôm chữ chứ không kéo hết cột), `$ cat motto.txt` → một câu về đam mê công nghệ và học hỏi, hàng bốn hành động (Email nền vàng `bg-highlight`, GitHub, LinkedIn, In CV) và ảnh chân dung **vuông** ở mép phải thân (override className vì `Avatar` bo tròn bằng class, không đọc `--radius`). Tên lệnh là **code** — `portfolio.hero.commands.*` giống hệt nhau ở hai locale, `test/messages.test.ts` ghim — và cả dòng lệnh `aria-hidden`, nên screen reader đi thẳng từ h1 sang chức danh. **Không** còn dòng "hiện đang làm ở đâu" (#274): nơi làm hiện tại là row đầu của Work ngay hai khối bên dưới, nên một dòng ở đây tiêu hàng được đọc nhiều nhất của trang cho thứ người đọc sắp thấy. Không còn Lens, không còn fade. |
| Ba section đáng nói | `src/features/home/components/` | **Work** là accordion (`resume-card.tsx`), mỗi hàng một khối tiêu chuẩn với logo vuông: chỉ row đầu mở sẵn, chevron luôn hiện — hover-only affordance thì trên điện thoại không tồn tại — và row Arobid mang badge giải VDA 2025 nền vàng (`bg-highlight`). Tên công ty, mốc thời gian, badge, tech stack là mono; chức danh và bullet là sans. **Projects** là **hai** khối tiêu chuẩn, một khối một dự án — không còn một khối chung gộp nhiều hàng (`project-row.tsx`, không còn card, không còn primitive `Card`, #269) — **không** còn câu ghi chú chung phía trên (#274 — nó nói hai dự án nằm trong monorepo cá nhân, điều mà chính href của mỗi link "Mã nguồn" đã ghi rõ): sau tiêu đề là thẳng hai khối. Mỗi khối: tên mono trên hàng riêng, một đoạn mô tả hai-ba câu sans, tech stack một dòng mono mờ (cap sáu tên ép ở tầng dữ liệu), rồi hàng link luôn ở cuối khối — không còn chia sẻ hàng với tên, dù tên dài ngắn khác nhau (#272) — **không** bullet, **không** badge loại. Cú **lún** mọi khối đều có: `StandardBlock` dịch 2px về phía bóng và rút bóng về 2px (`shadow-hard-pressed`), transform + shadow thôi nên hàng xóm không dịch, `motion-reduce` bỏ tween. Từ #124 mọi khối tiêu chuẩn lún, hero và About lẫn Skills — lún là chất liệu của trang, không phải dấu hiệu bấm được. **Skills** là **một** khối tiêu chuẩn chứa năm hàng dạng directory listing — nhãn `frontend/` mono bên trái (dấu `/` ẩn khỏi accessible name), tên kỹ năng là text mono cách nhau bằng dấu phẩy vẽ bằng pseudo-element, không còn badge — hiện cùng lúc, không phải tab: tab giấu bốn nhóm khỏi lần đọc đầu và khỏi crawler hoàn toàn. **About**, **Contact**, **Hobbies** mỗi cái một khối tiêu chuẩn; Contact mỗi dòng chỉ còn icon + giá trị sans, nhãn trường (`portfolio.contact.labels.*`) chuyển `sr-only` (#270 — cột nhãn cố định trước đây làm giá trị LinkedIn 36 ký tự xuống dòng), và Education là khối nhờ dùng chung `ResumeCard`. |
...
| Route module | `src/app/[locale]/(shell)/page.tsx` | `<PersonJsonLd />` + `<HomeTemplate />`, hết. Không `generateMetadata` riêng: title/description của root layout đã mô tả chính trang này, thêm một bản nữa chỉ tạo chỗ cho hai bên lệch nhau. |
| SEO | `src/app/[locale]/layout.tsx` + `components/person-json-ld.tsx` | `title` là **"Tên — Software Engineer"**, không phải tên trơ: tab, bookmark và kết quả tìm kiếm đều render đúng chuỗi này. `alternates` là thứ site hai ngôn ngữ không thể thiếu — `canonical` + map `languages` (kèm `x-default`) nói cho crawler biết `/` và `/en` là **một** trang ở hai tiếng, chứ không phải hai bản trùng để nó tự chọn; cả hai dựng bằng `~/utils/absolute-url.ts`, đúng helper `sitemap.ts` dùng, nên không có chỗ cho hai bên lệch prefix. `openGraph.locale`/`alternateLocale` nói lại điều đó cho unfurler. Và `PersonJsonLd` phát một `<script type="application/ld+json">` schema.org **Person** — nửa SEO mà `<meta>` không diễn đạt được: tên, chức danh, ảnh, email, địa chỉ, `sameAs` hai profile, `alumniOf`, `knowsAbout` — mọi giá trị đọc từ đúng nguồn trang hiển thị, không phải bản chép lại. |
...
| Cặp **highlight** vàng (hue 91–92) — từ #275 trang **không** còn vẽ gì bằng nó, vai duy nhất còn sống là mảng tên trên share card (Satori ghi literal, test giữ literal khớp token) | `highlight` + `highlight-foreground` | `#ffe14d` → `#eec743` dịu hơn; chữ trên nó gần đen ở cả hai theme, và focus ring trên nền vàng vẽ bằng chính `highlight-foreground` inset vì không vàng nào đạt 3:1 với `--ring` indigo trong dark |
...
`/` là **tiếng Anh** không prefix, `/vi` có prefix (`localePrefix: "as-needed"`
của `@monorepo/i18n`). Đây là app **duy nhất** đảo mặc định so với registry
(`defaultLanguage` vẫn là `vi`): CV được đọc bởi người có thể ở bất cứ đâu, và
bare path là thứ một người lạ không khớp `Accept-Language` — và một crawler —
nhận được. Khai đúng một chỗ, `defaultLocale: "en"` trong `~/i18n/routing.ts`;
mọi nơi khác trong app đọc `routing.defaultLocale`, **không** đọc
`defaultLanguage`.

Đi kèm là `localeDetection: false` — và đó mới là thứ làm mặc định *giữ lời*.
Để bật, next-intl đọc `Accept-Language` trước, nên trình duyệt tiếng Việt mở `/`
bị 307 sang `/vi` và "mặc định" hoá ra là ngôn ngữ của từng người. Tắt đi thì
chỉ URL quyết định: một link chia sẻ, một trang được crawl, một ngôn ngữ. Giá
phải trả: next-intl gộp việc đọc **cookie** vào cùng cờ này, nên lựa chọn của
lần trước không còn tự đưa người đọc khỏi bare path — muốn tiếng Việt thì dùng
switcher, và switcher đổi bằng cách điều hướng. Đổi ngôn ngữ là **điều hướng**, không phải mutate một
...
- `test/features/home/components/hero-section.test.tsx` — h1 là tên, dòng lệnh
  ...
  **ba** hành động toàn là link (nút In CV đang park — xem TODO trong
  `hero-section.tsx`), mailto không mở tab mới.
...
`StandardBlock` với viền 2px + bóng đặc, nút Email mặc cùng bộ với ba hành động
còn lại và **không** còn vệt vàng nào, ảnh vuông, card dự án hover không nhấc), `dock.e2e.ts` (khung vuông viền
