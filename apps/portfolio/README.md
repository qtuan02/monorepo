...
| Thứ | Ở đâu | Ghi chú |
| --- | --- | --- |
| Bố cục | `src/app/[locale]/(shell)/layout.tsx` + `templates/home.template.tsx` | Khung `max-w-6xl`. Một cột tới `md`; từ `lg` là **cột đọc** 2/3 (About, Work, Projects) + **rail** 1/3 dính khi cuộn (Skills, Education, Contact, Hobbies), hero trải cả hai. Đúng thứ tự DOM cắt đôi, nên test thứ tự section không đổi và tab order = thứ tự đọc. Projects hai cột từ `sm`, card lẻ cuối trải hết hàng; Skills trong rail xếp nhãn trên giá trị. `e2e/viewport.e2e.ts` đo cả hai bố cục. (#123 — v1 để cột 672px và ba card dự án bị ép vào ~200px.) |
