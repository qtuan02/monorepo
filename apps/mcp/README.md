...
```bash
bun run dev:mcp     # http://localhost:3004 — endpoint ở /api/mcp
```
...
| Key | Bắt buộc | Dùng ở |
| --- | --- | --- |
| `MCP_OPENWEATHERMAP_API_KEY` | **Có** | `src/features/weather/server/openweathermap.ts` |
| `NEXT_PUBLIC_MCP_SENTRY_DSN` | Không | `instrumentation.ts`, `instrumentation-client.ts` |
...
Đó cũng là lý do key ở đây là `MCP_OPENWEATHERMAP_API_KEY` chứ không phải
...
```bash
bun run dev:mcp                                  # dev server, cổng 3004
bun run build:mcp                                # build production

bun run --filter @monorepo/mcp typecheck         # next typegen && tsc
bun run --filter @monorepo/mcp test              # Vitest 5 + RTL (jsdom)
bun run --filter @monorepo/mcp test:coverage     # báo cáo v8, không có ngưỡng

bunx playwright test --project=chromium                  # E2E — chạy TỪ TRONG thư mục app
bun run e2e:headed:mcp                           # cùng spec, một cửa sổ thật
...
"buildCommand":   "cd ../.. && npx --yes bun@1.4.0 run --filter @monorepo/mcp build:vercel",
```
...
| `MCP_OPENWEATHERMAP_API_KEY` | app | **Có** |
| `NEXT_PUBLIC_MCP_SENTRY_DSN` | app | Không |
...
DSN đọc từ `NEXT_PUBLIC_MCP_SENTRY_DSN` và truyền vào `initSentryClient`
