 ...
 * icon-only search and a menu button, and the rest moves into the sheet.
 *
    ...
    <div className="sticky top-4 z-40 flex justify-center px-4">
      <nav className="glass flex w-full max-w-[980px] items-center gap-1.5 rounded-full py-2 pr-2.5 pl-3">
        ...
        <Button
          ...
          className={cn(
            roundControlClassName,
            "text-muted-foreground ml-auto px-0 md:w-64 md:justify-start md:gap-2.5 md:pr-2 md:pl-3.5",
          )}
        >
          <SearchIcon className="size-4 shrink-0" />
          <span className="hidden truncate text-[13.5px] font-normal md:inline">
            {t("documents.nav.search.label")}
          </span>
          <kbd className="bg-card text-foreground/80 border-foreground/15 ml-auto hidden rounded-md border px-1.5 py-0.5 font-mono text-[11px] md:inline">
            {shortcutLabel()}
          </kbd>
        </Button>
