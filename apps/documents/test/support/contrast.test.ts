...
/**
 * shadcn and the workspace theme write every colour in OKLCH; the hex beside
 * each case is the sRGB the value was converted from (the first two are the
 * poles, the rest the EMR palette the theme once carried). Those pairs are an
 * oracle this file did not produce: if `oklchToRgb` walks OKLab → linear sRGB
 * → the sRGB transfer curve correctly, it has to land back on the hex to the
 * byte.
 */
describe("OKLCH → sRGB", () => {
  it.each([
