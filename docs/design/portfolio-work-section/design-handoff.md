...
| Pill khoảng thời gian + chấm (vai trò hiện tại) | **cần thêm — composite của slice** `~/features/home/components/current-period-pill.tsx` | `bg-accent text-accent-foreground rounded-full`; **không** dùng `@monorepo/ui/components/badge` — badge `base-vega` không có variant accent và sẽ phải tắt lại `border`/`px` của nó |
| Chip công nghệ | **cần thêm — composite của slice** `~/features/home/components/tech-chip.tsx` | `bg-secondary text-muted-foreground rounded-sm`; cùng lý do với pill |
| Nhóm chip có tràn `+n công nghệ` | **cần thêm — composite của slice** `~/features/home/components/tech-chip-list.tsx` | nhận `techStack` + số chip hiện; xem §6 câu hỏi 1 |
...
| Thân gập/mở | **có sẵn** — `motion.div` trong `resume-card.tsx` | xem §6 câu hỏi 3 |

**Không cần `ui-add` primitive nào.** Ba composite mới đều thuộc slice `home`, không phải `~/components`:
chỉ section này dùng chúng, và `architecture-shared-components` giữ `~/components` cho thứ **nhiều hơn
một** slice dùng.
...
| `oklch(0.9642 0 0)` nền chip | `--secondary` | `bg-secondary` |
| `9999px` | — | `rounded-full` |
| `6px` | `--radius-sm` | `rounded-sm` |
| font stack | `--font-sans` | mặc định |
| dark: `0.145` · `0.985` · `0.205` · `0.708` · `1 0 0 / 10%` · `0.269` · `0.7893 0.0875 183.49` · `0.35 0.0382 184.2` | `--background --foreground --card --muted-foreground --border --secondary --primary --accent` ở `.dark` | tự đổi theo class |

Direction A **dùng** bốn token trước đây section này chưa chạm — `--primary`, `--accent`,
`--accent-foreground`, `--secondary` — nhưng cả bốn đã có sẵn trong theme port từ web-emr. **Không có
dòng "thêm", không có dòng "đổi"**, nên topic này **không** cần ticket token delta và **không** cần ADR.

**Ba giá trị không phải token nhưng cũng không nằm trên scale Tailwind** — delta *type scale*, không phải
