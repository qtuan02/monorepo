...
/**
 * What both consumer smoke tests share (ADR-0004).
 *
 * `scripts/publish-smoke.ts` runs *before* a release, against tarballs `npm
 * pack` produced from the working tree; `scripts/verify-release.ts` runs
 * *after* one, against what npm actually serves. The two differ only in where
 * the packages come from — the throwaway consumer project, the assertions on
 * the installed files, and the assertion on the CSS it builds are the same
 * test, so they live here instead of being kept in sync by hand.
 */
...
export type Shell = {
   ...
   * to prove a stylesheet shipped in a package reached the consumer's Tailwind
...
export const SHELLS: readonly Shell[] = [
  ...
  },
  {
    ...
    // one surviving into `dist/` is a package that installs and then fails to
    // resolve.
...
export const FORBIDDEN_IN_MANIFEST = ["catalog:", "workspace:"] as const;

/** Never allowed in any published `dist/`, whichever shell it belongs to. */
export const FORBIDDEN_IN_DIST = ["@monorepo/"] as const;

/**
 * What the throwaway consumer installs beside the shells. These pins restate
...
/** The monorepo root. This file sits at `scripts/lib/`, two levels down. */
 ...
 * path under `C:\Users\First Last\...` would arrive as two. Quoting each
 * argument is what keeps a temp directory with a space in it working.
 */
export const useShell = process.platform === "win32";

export function shellSafe(args: string[]): string[] {
...
/**
 * Prints every failed assertion and exits non-zero, or prints `okMessage`.
 * Both scripts end this way, so a non-zero exit means the same thing in CI
 * whichever one ran.
 */
export function reportFailures(okMessage: string): void {
...
/**
 * Asserts on the installed package rather than on the tarball or on registry
 * metadata: what Bun unpacked into `node_modules` *is* the published contents,
 * and it is also what the consumer's typecheck and build below actually
 * resolve. `origin` only names where it came from, in the log line.
 */
export async function assertInstalledShell(
  shell: Shell,
  consumerRoot: string,
  origin = "its tarball",
): Promise<void> {
  ...
  if (!existsSync(installed)) {
    check(false, `${shell.name} installed from ${origin}`);
    return; // every assertion below reads a file inside it
  }
  check(true, `${shell.name} installed from ${origin}`);
...
/**
 * Writes the throwaway Vite + React 19 project. It declares every dependency a
 * consumer would pick for themselves, but never the shells — those are added
 * afterwards, from a tarball or from the registry depending on the caller.
 */
export async function scaffoldConsumer(root: string): Promise<void> {
