...
│   │   ├── src/features/       ← two slices: `home` (the CV itself — eight sections in reading order, Hero → About → Work → Projects → Skills → Education → Contact + Hobbies, plus `constants/resume.ts`, the structure that joins to the `portfolio.*` message namespace) and `layout` (the dock navbar, the theme provider, the theme toggle). The order is an argument rather than a layout detail and is pinned in `test/features/home/templates/home.template.test.tsx` — two tickets adding a section in parallel is how it drifted once, silently, since every section still rendered
│   │   ├── src/app/            ← `[locale]/(shell)/` with a **one-line** `page.tsx`, plus `[locale]/opengraph-image.tsx` (the social card is **generated per locale** rather than a static `og-image.jpg`, so the `/en` share card is English with no second file) and the three App Router metadata routes `manifest.ts` · `robots.ts` · `sitemap.ts` (the sitemap carries the `vi`/`en` `alternates.languages`, which is why `NEXT_PUBLIC_PORTFOLIO_BASE_DOMAIN` is required rather than optional — and so does the card, whose `og:image` has to be absolute)
│   │   ├── src/globals.css     ← the three things a CV needs that the shared theme does not ship: a **seven-token indigo accent** overriding the EMR palette's teal, written **unlayered** because `theme.css` arrives through a bare `@import` and an unlayered declaration beats one in any `@layer`; the `prefers-reduced-motion` branch, which is what makes the content visible with **no JavaScript at all** (the JS branch in `BlurFade` only answers after hydration, by which time the server has already sent `motion`'s inline `opacity: 0`); and `@media print`, because this page **is** the CV and the hero's "download" is the browser's own print dialog
