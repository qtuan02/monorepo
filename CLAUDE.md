...
│   ├── mcp-weather/             ← `@monorepo/mcp-weather` — a **Model Context Protocol** server over OpenWeatherMap, and the one app here whose product is an endpoint rather than a screen. Runtime **Next** (cloned from `_template_next`), dev 3004 / E2E 3104 in its `ports.env`. It replaces `legacy/mcp` **without changing the wire contract** — `POST /api/mcp`, Streamable HTTP, three tools (`hello-world`, `get-weather`, `get-forecast`), no auth — because a backend outside this repo already calls it
...
│   ├── assistant-ai/            ← `@monorepo/assistant-ai` — the Gemini chat app, and the one app here that both *calls* another app in this repo and takes a whole third-party SDK family to its latest major in one go. Runtime **Next** (cloned from `_template_next`), dev 3005 / E2E 3105 in its `ports.env`. It replaces `legacy/assistant-ai`
│   │   ├── src/app/api/chat/route.ts ← the browser's transport posts here. Thin like every route module: validate `model` and `locale` out of an untrusted body, resolve the catalogue for that locale, hand both to the slice. Outside `[locale]`, and `proxy.ts`'s matcher excludes `/api` — which is exactly why the locale has to travel **in the body** rather than in the URL
│   │   ├── src/features/chat/  ← the one business slice. `server/chat-model.ts` builds the provider with `createGoogleGenerativeAI({ apiKey: env.… })` rather than importing the ready-made `google` instance, because that instance reads `process.env` itself and would put the app's one indispensable value outside `~/env.ts`. `server/mcp-tools.ts` opens a `@modelcontextprotocol/sdk` client per request (the server it calls is stateless, so there is no session to keep) and hands each tool's JSON Schema straight to `ai`'s `jsonSchema()` — the 130-line hand-written JSON-Schema→zod walker the old app carried is gone, along with the `z.any()` fallback that silently widened a required enum. `server/stream-chat.ts` adds `stopWhen: stepCountIs(5)` whenever tools are present, so the model writes a summary *after* a tool returns instead of ending the turn on raw JSON. `utils/chat-error-code.ts` is the pure half: it classifies a failure into a code and the catalogue supplies the sentence, so a rejected key and an exhausted quota stop looking identical
│   │   ├── src/constants/models.ts · src/stores/use-model-store.ts ← the model list sits in `~/constants` and not in the slice, because the **global** store reads it and a store may not import from a feature. The store persists the chosen id and nothing else — a model id is public, and this app's session is still an `HttpOnly` cookie
│   │   ├── src/env.ts          ← the Next Flavor with three app-owned keys — `GOOGLE_GENERATIVE_AI_API_KEY` (server, **required**, and the one key in the workspace that keeps a bare name, because `@ai-sdk/google` owns it), `ASSISTANT_AI_MCP_DOMAIN` (server, optional: without it the app is a plain chat rather than a boot failure) and `NEXT_PUBLIC_ASSISTANT_AI_SENTRY_DSN`
│   │   └── **The one shape worth knowing before editing it**: the chat surface renders inside a `<Suspense>` in `page.tsx`, and that is load-bearing rather than cosmetic — the assistant-ui runtime mints message ids with `Math.random()` during render, which under `cacheComponents` fails `next build` outright. So what the server sends a crawler is the shell plus a skeleton, and `e2e/server-rendering.e2e.ts` asserts that rather than chat content
...
bun run dev:assistant-ai         # the Gemini chat (Next)  — http://localhost:3005
...
bun run e2e                      # Playwright over every app with an `e2e` task — both Templates, portfolio, documents, mcp-weather, assistant-ai; each webServer builds and serves itself
...
bun run e2e:headed:assistant-ai
