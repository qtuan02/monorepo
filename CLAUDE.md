...
│   ├── documents/               ← `@monorepo/documents` — the reference site for the two **published** packages, written for an npm consumer rather than for this repo, so every example names `@fe-monorepo/ui` / `@fe-monorepo/hook` and never the workspace name. Runtime **Vite** (cloned from `_template_vite`), dev 3003 / E2E 3103 in its `ports.env`. **Redesign "Prism" decided 2026-09-16, not yet implemented** (brief `docs/design/documents-redesign.md` §2c/§10, ADR-0009, glossary `apps/documents/CONTEXT.md`): no sidebar — a sticky glass nav pill, a `⌘K` `CommandDialog` and prev/next links are the navigation — glass panels over a static aurora + geometric backdrop, an indigo palette overriding the shared theme at the app layer, Outfit + JetBrains Mono via `@fontsource-variable`, dark mode as "indigo night". Until the six tickets land, the tree below describes the shipped app
