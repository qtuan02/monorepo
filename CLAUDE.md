...
│   │   ├── src/utils/          ← `currency.ts` (`vi-VN` VND) · `date.ts` (`@monorepo/dayjs` + `DATE_FORMAT`) · `pagination.ts` (`getTotalPages` ≥ 1, `clampPage` two-sided, `getPageItems`, `paginate`, the page-size table)
