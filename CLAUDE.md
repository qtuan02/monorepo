...
│   │   ├── src/env.ts          ← the Next Flavor with three app-owned keys — `GOOGLE_GENERATIVE_AI_API_KEY` (server, **required**) and `MCP_DOMAIN` (server, optional: without it the app is a plain chat rather than a boot failure), both keeping their bare names for the reasons its README gives, plus `NEXT_PUBLIC_ASSISTANT_AI_SENTRY_DSN`
