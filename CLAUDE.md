...
│   │   ├── src/components/     ← the **composites every slice stands on** (#133), each on a `@monorepo/ui` primitive: `data-table/` (`data-table.tsx` — search, faceted filters, sort, page + page size over one instance from `useDataTable`, with search/facets/page/size on the **URL** through `use-table-search-params.ts` and `useSearchParams`, no `nuqs`; `search-input.tsx` debounced through `@monorepo/hook/use-debounce`; `faceted-filter.tsx`; `pagination-bar.tsx`, the ONE pagination) · `badge/status-badge.tsx` (the ONE status badge, fed a `StatusConfig`) · `page/` (`list-page-header`, `detail-page-shell`) · `panel/` (`empty`, `error`, `loading`) · `card/` (`summary`, `info` + `InfoRow`, `entity-list`, `stat-item`) · `dialog/confirm-action-dialog` · `menu/entity-action-menu` · `navigation/page-back-button`
│   │   ├── src/constants/status.ts ← the ONE home for status/display config: `statusTone`, `StatusConfig`, `roomStatusConfig`, `roomTypeConfig`, `toFilterOptions()` — later slices append theirs here
│   │   ├── src/utils/          ← `currency.ts` (`vi-VN` VND) · `date.ts` (`@monorepo/dayjs` + `DATE_FORMAT`) · `pagination.ts` (`getTotalPages` ≥ 1, `clampPage` two-sided, `getPageItems`, the page-size table)
│   │   ├── src/features/       ← `auth` (fake sign-in: a schema-valid form sets a token; `register` navigates to onboarding; `provider/` guards as the Template) · `layout` (the Portal shell — `sidebar` primitive with the 15 areas in three groups + nav-user, a header with the Building scope selector, the notification popover and a ⌘K `command` palette; `constants/navigation.ts` is the manifest the sidebar and the header title both read, `utils/navigation.ts` the segment-boundary match) · `buildings` (card grid + `building-form-dialog` on Zod — `types/building-form.ts` parses the form's strings into the request's numbers — and the detail screen; `utils/building-stats.ts` derives the two figures the Mock lacks) · `rooms` (the first `DataTable` consumer — `components/room-columns.tsx` on the column helper, `room-grid.tsx` the card view by floor, the view itself a `view=` URL param; the detail screen deletes through `useDeleteRoom` behind the confirm dialog) · one slice per remaining domain, each holding a placeholder template until its ticket ports the screen
│   │   ├── src/stores/         ← `use-auth-store.ts` (token + `user` for nav-user) · `use-building-store.ts` (**Building scope**, `persist`, `null` = every Toà nhà)
│   │   └── `~/libs/http-client.ts` exports only `httpClient` — no service class until `be-motel` has a contract; reads are Mock constants behind `~/hooks/api` (`building.ts` is the shape every slice copies; `room.ts` adds the Building scope as a typed list param and a delete mutation that splices the Mock). The Mock itself lives in **`~/constants/mock/<entity>.ts`**, not in the slice the spec named: `~/hooks/api` serves it and a hook may not import `~/features`
