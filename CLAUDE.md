...
│   ├── chat/                    ← `@monorepo/chat` — a real-time Messenger-style app, ported 1:1 from `chat-socket-fe` (Rsbuild + STOMP, `D:\Personal\chat\chat-socket-fe`) onto backend `chat-socket` (Spring Boot, REST + STOMP on one port). Spec [#195](https://github.com/qtuan02/monorepo/issues/195) — phase 1, no redesign. Glossary [`apps/chat/CONTEXT.md`](./apps/chat/CONTEXT.md) — Session (an in-memory access token + an `HttpOnly` refresh cookie, a third shape beside the Vite Template's persisted token and the two SSR Runtimes' cookie session — ADR-0007), Health gate, Presence, Conversation (direct/group), Draft conversation, Friend request. Runtime **Vite** (cloned from `_template_vite`), dev 3007 / E2E 3107 in its `ports.env`. Two env keys of its own — `PUBLIC_CHAT_API_BASE_URL` (the `chat-socket` origin; the service singleton appends `/api`) and `PUBLIC_CHAT_SOCKET_URL` (the STOMP-over-WebSocket URL) — via `baseEnvSchema.extend()`. Refresh-and-retry on a 401/403 is solved once in `@monorepo/api` rather than a second HTTP layer — [ADR-0014](../../docs/adr/0014-api-refresh-token-opt-in-http-client.md), `createHttpClient`'s opt-in `withCredentials`/`onAuthError`. `src/globals.css` keeps the source's own teal palette as an app-layer override (`--primary`/`--ring`/`--sidebar-primary`/tinted sidebar, `oklch()` kept rather than converted to hex) plus a token this app introduces, `--online`/`--color-online`, for the Messenger-style presence dot — same unlayered-`:root`/`.dark` shape as `portfolio` (ADR-0008), `documents` (ADR-0009) and `smart-rental` (ADR-0011). Deployed on Vercel like `smart-rental`. **What it deliberately does not have**, recorded in its README: no i18n (hardcoded English copy, `setDayjsLocale("en")` once in `src/index.tsx`, `@monorepo/i18n` off its deps), no `home` slice, no `~/services/` (every backend call goes through a service class in `@monorepo/api`), no redesign. Four npm packages the source app depends on (`@stomp/stompjs`, `react-virtuoso`, `@emoji-mart/react`, `@emoji-mart/data`) sit in the root Bun catalog, imported once the real socket/message/emoji tickets land
