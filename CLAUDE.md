...
│   │   ├── src/globals.css     ← the three things a CV needs that the shared theme does not ship: a **seven-token indigo accent** overriding the EMR palette's teal, written **unlayered** because `theme.css` arrives through a bare `@import` and an unlayered declaration beats one in any `@layer`; the `prefers-reduced-motion` branch, which switches off by name the one animation CSS still owns — the theme wipe — now that no section fades in, the hero no longer waves and nothing on the page starts at `opacity: 0`; and `@media print`, because this page **is** the CV and the hero's "download" is the browser's own print dialog
