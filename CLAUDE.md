...
│   ├── assistant-ai/            ← `@monorepo/assistant-ai` — the Gemini chat app, and the one app here that both *calls* another app in this repo and takes a whole third-party SDK family to its latest major in one go. Runtime **Next** (cloned from `_template_next`), dev 3005 / E2E 3105 in its `ports.env`. It replaces the legacy `assistant-ai` app
...
This project is indexed by GitNexus as **monorepo** (3957 symbols, 7914 relationships, 214 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
