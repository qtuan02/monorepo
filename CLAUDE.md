...
│   ├── documents/               ← `@monorepo/documents` — the reference site for the two **published** packages, written for an npm consumer rather than for this repo, so every example names `@fe-monorepo/ui` / `@fe-monorepo/hook` and never the workspace name. Runtime **Vite** (cloned from `_template_vite`), dev 3003 / E2E 3103 in its `ports.env`. Its visual shape is **"Prism"** (spec #128, shipped 2026-09-17; brief `docs/design/documents-redesign.md` §10, ADR-0009, glossary `apps/documents/CONTEXT.md`, the shipped account in its README § Hình dạng): **no sidebar** — a sticky glass nav pill, a `⌘K` `CommandDialog` over both catalogues and prev/next links in catalogue order are the whole navigation — every content surface a glass panel over a static aurora + geometric backdrop (`full` on `/`, `soft` elsewhere, shapes hidden below `md`), list pages as a `1/2/3/4`-column grid of opaque tiles (below `sm` a tile is one row — swatch left, slug + preview stacked, count at the end, one DOM for every breakpoint) with a slug-derived swatch and a two-column tile from ten exports, an indigo palette overriding the **whole** shared palette at the app layer, Outfit + JetBrains Mono via `@fontsource-variable`, and a dark mode that is "indigo night" rather than the theme's grey — the second app to override the theme, for its own reason. Responsive round (spec #215, shipped 2026-09-19; brief `docs/design/documents-responsive.md`) collapses the nav pill's search box to an icon button below `lg` (three links and four round buttons stay to `md`, a `Sheet` below it), makes the detail hero's swatch sit beside its `h1` on one `grid-cols-[auto_minmax(0,1fr)]` row at every breakpoint instead of stacking above it (shrinking to 64px with the two actions dropping to a full-width `h-10` stack below `sm`), and anchors the `⌘K` palette to the top of the viewport with its subpath hidden below `sm` so the virtual keyboard never covers a result
...
│   │   ├── src/features/       ← `getting-started` · `component` · `hook` · `layout` (the nav pill, the search palette, the backdrop, the theme provider — a context, an effect and one `localStorage` key, no store). `~/components/` holds the Prism vocabulary as composites: `panel/glass-panel`, `tile/tile` (renders its own `<li>`, because that is the grid item the `md:col-span-2` has to sit on), `swatch/swatch`, `detail/{toolbar,hero,panels}`, `page/{list-header,docs-section}`. `~/pages/` holds `main.tsx` plus six thin wrappers over the route table in `~/constants/routes.ts` (`/components/:slug`, `/hooks/:slug` and their builders); an unknown slug 404s in place rather than redirecting
...
│   ├── portfolio/               ← `@monorepo/portfolio` — the personal CV site. Runtime **Next** (cloned from `_template_next`) because the whole point is a public page a crawler reads before any JS runs. dev 3002 / E2E 3102 in its `ports.env`. Its visual shape is the **v2 redesign — Terminal / neubrutalist** (spec #113, `docs/design/portfolio-redesign-v2.md` §7, ADR-0008): square corners, hard 2px edges, a solid offset shadow, mono for headings and labels over sans for prose, two accents on near-black/near-white neutrals, an inverted dark theme, and **no scroll or load fade** — the only app in the workspace that overrides the shared neutrals, on purpose. Responsive down to 320px (spec #210, `docs/design/portfolio-responsive.md`): the dock becomes a full-width **bottom bar** below `sm`, the hero and each `ResumeCard` reflow to one grid column on a phone with a 2×2 action grid, and from `md` the page is a **3/2** read-column/rail split (2/1 from `lg`, unchanged)
│   │   ├── src/features/       ← two slices: `home` (the CV itself — eight sections in reading order, Hero → About → Work → Projects → Skills → Education → Contact + Hobbies, plus `constants/resume.ts`, the structure that joins to the `portfolio.*` message namespace) and `layout` (the dock navbar, the theme provider, the theme toggle). The order is an argument rather than a layout detail and is pinned in `test/features/home/templates/home.template.test.tsx` — two tickets adding a section in parallel is how it drifted once, silently, since every section still rendered. Every section after the hero renders on **one** `components/standard-block.tsx` (2px `--border`, `shadow-hard`, `rounded-none`, `bg-card`; it carries its own `print:` half — 1px edge, no shadow). The **hero is the page's one terminal window**: the same block with a title bar (`aria-hidden`, `print:hidden`) and a body in shell order — `$ whoami` → the name as the **h1** (no greeting, no emoji), `$ cat role.txt` → positioning, `$ current --job` → the current role, four actions with the Email one on the yellow `bg-highlight`, and the portrait cut square by className because `Avatar` rounds by class, not by `--radius`. The command names are code: identical in both locales, pinned by `test/messages.test.ts`. The dock in `layout` is a **square, 2px-edged, hard-shadowed frame with no magnification** — the bar sinks toward its shadow on hover, a control drops one pixel on click — carrying four link items, the theme toggle (the wipe stays) and, since #124, the language switcher (`~/components/select/select-language.tsx`, restyled by className into the bar). From `lg` the CV is **two columns** (#123), a sticky rail beside the reading column, and the theme is the slice's own `provider/theme-provider.tsx` (#126) rather than `next-themes`
│   │   ├── src/app/            ← `[locale]/(shell)/` with a **one-line** `page.tsx`, plus `[locale]/opengraph-image.tsx` (the social card is **generated per locale** rather than a static `og-image.jpg`, so the `/en` share card is English with no second file — the route module only resolves locale, catalogue and host, and `features/home/components/open-graph-card.tsx` draws it in the page's own grammar: a hard-edged, hard-shadowed white block on the page ground, `$ whoami`, the name on a yellow slab, no radius, still Geist with no `fonts` so `next build` stays offline; `OPEN_GRAPH_PALETTE` spells the light theme's sRGB literally because Satori reads no custom property, and a test holds each literal to its token) and the three App Router metadata routes `manifest.ts` · `robots.ts` · `sitemap.ts` (the sitemap carries the `vi`/`en` `alternates.languages`, which is why `NEXT_PUBLIC_PORTFOLIO_BASE_DOMAIN` is required rather than optional — and so does the card, whose `og:image` has to be absolute)
│   │   ├── src/globals.css     ← the three things a CV needs that the shared theme does not ship: a **palette of its own** — twelve colour tokens and one length (v1's seven-token indigo accent, the two neutrals `--foreground`/`--border` v2 pushes to the poles, the yellow **highlight pair** for exactly two roles, `--hard-shadow`, and `--radius: 0px` **with a unit**) written **unlayered** because `theme.css` arrives through a bare `@import` and an unlayered declaration beats one in any `@layer`, the dark theme being the light one **inverted**; the `prefers-reduced-motion` branch, which switches off by name the one animation CSS still owns — the theme wipe — now that nothing fades, waves or magnifies; and `@media print`, because this page **is** the CV and the hero's "download" is the browser's own print dialog, keeping only what a utility cannot say while a block's own print half lives beside its screen classes. The token contract is `test/globals.test.ts` — extend it on purpose, never loosen it; the full account of each token and every ratio is the README's palette section and ADR-0008, not this line
│   │   ├── src/proxy.ts        ← the file, the named `export function proxy` and the literal `config.matcher` are all unchanged, and the body is `negotiateLocale(request)` behind one pass-through: no route here is guarded, so the Template's session branch is gone rather than kept with an empty prefix list pointing at a sign-in screen that does not exist, and a generated metadata image (`/vi/opengraph-image`) skips negotiation because `as-needed` would 307 the default locale's card through a redirect a link unfurler may not follow — the decision is the pure `~/utils/metadata-image-path.ts`. The guard mechanism itself still lives in `_template_next` and comes back with the next `gen:app`
...
│   │   ├── src/env.ts          ← the Next Flavor with two app-owned keys — `MCP_OPENWEATHERMAP_API_KEY` (server, **required**: the two weather tools are the whole app, so a missing key must fail `next build` by name) and `NEXT_PUBLIC_MCP_SENTRY_DSN`
│   │   └── **What it keeps that `portfolio` dropped**: `[locale]`, next-intl, `proxy.ts` *with* the session guard, the `auth` slice and the guarded `dashboard` — the ticket asked for the Template kept intact, and `/api/mcp` is outside the matcher either way. What it replaces is the Template's `home` slice: the public page is now an overview of the endpoint, under the `mcp.*` message namespace
│   ├── smart-rental/            ← `@monorepo/smart-rental` — the landlord's **Portal** for managing rented rooms. Phase 1 (spec #127) ported the prototype `fe-motel-rsbuild` onto Mock; phase 2 (spec #153, glossary `apps/smart-rental/CONTEXT.md` — Portal, Mock, Toà nhà, Building scope, Kỳ, Ngày thu, Chỉ số điện nước, Việc cần làm…) redesigned it into **"Hôm nay"**; round 3 (spec #179, ADR-0013) — **"dễ hơn"** — shipped: the chốt chỉ số → lập Đợt hoá đơn → Thu tiền chain collapses into one screen (`/cycles/:month`), Hôm nay's queue gộp Hoá đơn quá hạn by Toà nhà and sorts by hạn instead of by source, the sidebar regroups around that same chain, the Hợp đồng wizard drops to 2 steps, and Thu tiền is one click with the amount pre-filled. Spec #227 then deepened four slices independently: `MeterCell` (`~/features/cycles/components/`) is the one gate shared by the Kỳ table and its mobile card, `contractActions` (`~/utils/contract-status.ts`) is the one Gia hạn/Thanh lý/Xoá decision every Hợp đồng consumer reads instead of re-deriving it, `taskTypeConfig` in `~/constants/status.ts` is the one icon/label/route table the bell and Hôm nay's queue both read, and the Mock now stores exactly one date encoding — ISO (`YYYY-MM-DD` dates, ISO timestamps, `YYYY-MM` kỳ) — with every screen converting to `DD/MM/YYYY`/`MM/YYYY` only at render via `~/utils/date.ts`'s `formatDate`/`formatMonth`/`formatOptionalDate`. Round 4 (spec #241, brief `docs/design/smart-rental-round4.md`) — **"gọn hơn"** — shipped: no IA/flow/token change, only *how many things* a screen says and *how big* — cut per composite across 7 tickets + one tổng kiểm (#242–#249). `AppHeader` drops the sidebar item's title/description from `md` up (trigger · search · bell · avatar only), `ListPageHeader`/`DetailPageShell`'s own `h1`/`h2` is the one heading left, and every size collapses to **four tiers** (trang 20/600 · mục 15/600 · thân 14/400 · meta 12/400, money the one exception at 600 outside "thân") — written as `text-[20px]`/`text-[15px]` so `test/text-tier-guard.test.ts` can text-scan `src/components/**` + `src/features/**/templates/**` for the banned `text-2xl`/`text-xl`/`font-bold` with no allowlist. `DataTable` became the one owner of the entity count ("N `entityLabel`", or "M / N" while filtered) and auto-hides its search box and `PaginationBar` once `total ≤ pageSize`; its row is `h-11` (44 px, down from 53). `InfoCard`'s `InfoRow` (flex-between, label and value up to 600 px apart) became a `dl` two-column grid; `DetailPageShell`'s right column only renders when a screen has an action beyond its own header, and the "Liên kết" card is gone everywhere. `KpiStrip` went to `p-3`/`text-lg font-semibold` with a mobile 2×2 grid; a `*-columns.tsx` file carries no decorative `lucide-react` icon and no `text-primary` money (`text-foreground font-semibold tabular-nums text-right` instead); the Phòng card shrank from 235 px to ≈ 96 px and the Toà nhà card dropped its empty `h-32` image block; the Kỳ table gained a two-tier header ("Điện (kWh)" → "Cũ · Mới · Dùng") with `MeterCell` (#228) still one gate, now rendering 3 `td`s instead of one flex cell; Báo cáo's chart colour moved to `chartConfig`'s `var(--primary)` and dropped its donut for a labelled bar; Đối soát went back to a plain `Table`. One screen keeps at most one navy `data-variant="default"` button — everything else is `outline`/`ghost`. Full app shape after round 4 — `apps/smart-rental/README.md` § Hình dạng round 4; the 22 decisions and what shipped vs. deviated (mobile Hoá đơn's first row lands at 402 px, short of the ≤ 360 px target — tracked in [#250](https://github.com/qtuan02/monorepo/issues/250)) — comment on spec [#241](https://github.com/qtuan02/monorepo/issues/241). Runtime **Vite** (cloned from `_template_vite`), dev 3006 / E2E 3106 in its `ports.env`, no env key of its own. Deployed on **Vercel** like `documents` — `vercel.json` (install/build via `npx --yes bun@1.4.0` from the repo root, SPA rewrite), the three base `PUBLIC_*` keys from the dashboard; the Template Dockerfile stays for the CI `docker` job. The prototype repo `fe-motel-rsbuild` is archived. **What it deliberately does not have**, recorded in its README and not drift to synchronise back: no i18n (hardcoded Vietnamese copy, `setDayjsLocale("vi")` once in `src/index.tsx`, `@monorepo/i18n` off its deps), no dark-mode **toggle** (the token pair is written, unused — see below), no `home` slice (`/` is "Hôm nay"), no sign-up/onboarding (removed in phase 2 — a fake sign-in is the whole auth surface), no manual Task entry (Việc cần làm is entirely derived; round 3 dropped its own `/tasks` screen too — it lives only in Hôm nay's queue and the bell), and no `paymentDueDay`/`utilityCycleDay` (round 3 collapsed both onto `Building.collectionDay`, "Ngày thu" — a maintenance-ticket slice waits on a real `maintenance-service`)
│   │   ├── src/globals.css     ← [ADR-0011](../../docs/adr/0011-smart-rental-ledger-palette-override.md): an unlayered `:root`/`.dark` block overriding the shared theme at the app layer, the same move `portfolio` (ADR-0008) and `documents` (ADR-0009) make — navy `--primary`/`--ring`/`--sidebar-primary`, `--radius: 0.375rem`, IBM Plex Sans via `@fontsource-variable`. `.dark` ships fully written but is never toggled to; `--success`/`--warning`/`--info`/`--destructive` and every `--chart-*`/`--sidebar-*` stay the theme's own, so a status colour never changes meaning between apps — round 3 (spec #179) adds four app-owned `--{success,warning,info,destructive}-foreground-strong` tokens beside them, read only by `statusTone`'s badge text so it clears WCAG AA on the `/10` background. `test/globals.test.ts` (contrast + token-parity, copied from `portfolio`) is the gate
│   │   ├── src/constants/routes.ts ← the one path table + builders, read from `~/pages/main.tsx`'s `AppRoutes` (split from `MainApp`, the providers + `BrowserRouter`, so `test/pages/main.test.tsx` can mount `AppRoutes` alone in a `createMemoryRouter`). That file is **the spec's one seam**: it mounts the tree at every `ROUTES` path (with and without a Building scope) and asserts the screen's `h1` **and one derived fact** (a Hôm nay task, an overdue badge, a status suy ra) — the guard cases (`replace` proven through `router.state.historyAction`, an unknown path 404ing) sit beside them. Every domain ticket added its own rows here
│   │   ├── src/components/     ← the **composites every slice stands on** (#133, deepened for phase 2's one screen grammar): `data-table/` (`data-table.tsx` — search, faceted filters, sort, bulk-selection sticky action bar, `renderMobileRow` for a card-per-row phone layout, a `ToggleGroup` card/table switch, page + page size over one TanStack instance from `useDataTable`, search/facets/page/size on the **URL** via `use-table-search-params.ts`; `pagination-bar.tsx`, the ONE pagination) · `card/` (`kpi-strip` — one strip, two sizes, replacing `summary-card`; `info-card` + `InfoRow`; `entity-list`; `stat-item`) · `badge/status-badge.tsx` (the ONE status badge, fed a `StatusConfig`) · `page/` (`list-page-header`; `detail-page-shell` — one entity header + real tabs + a right action column, with a `Breadcrumb` at route depth ≥ 3 in place of a bare back button, and a `query` + `notFound` + `children(entity)` so no detail template hand-writes its own loading/not-found branch; `relation-tab` — a tab's own card grid or `EmptyPanel`, fed `items` + `children(item)`) · `panel/` (`empty`, `error`, `loading` — footprint-shaped skeletons, never one generic spinner) · `form/` (`sheet` short forms, `date-field`, `month-field`/`month-picker` for a kỳ, `currency-input-group`, a self-fetching `combobox`, `attachment`) · `dialog/confirm-action-dialog` · `menu/entity-action-menu` (an item with neither `link` nor `onClick` renders disabled — never ship one that always is) · `queue/task-queue` (Hôm nay's việc cần làm rows — Hoá đơn quá hạn of one Toà nhà collapse into one `Collapsible` mục, sorted by hạn; round 3 dropped the `/tasks` screen that used to share this) · `chart/revenue-chart` (moved into the `reports` slice, round 3) · `navigation/page-back-button`
│   │   ├── src/constants/status.ts ← the ONE home for status/display config: `statusTone`, `StatusConfig`, per-entity configs (room, contract, deposit, invoice, invoice payment method, utility, supplier bill, task, communication channel…), `toFilterOptions()` — every slice appends its own here
│   │   ├── src/utils/          ← pure derivation only (ADR-0012), each with a unit test: `currency.ts`, `date.ts`, `pagination.ts`, `csv.ts` (`toCsv`, RFC 4180), `vietqr.ts` (the `img.vietqr.io` quick-link + `addInfo` truncation), `room-delete.ts` / `contract` lifecycle helpers, `invoice-payments.ts`, `task-due.ts`, `report-rows.ts`, `reconciliation-items.ts`, `mock-reset.ts` (`trackMockReset` — the primitive "Khôi phục dữ liệu mẫu" is built from)
│   │   ├── src/features/       ← `auth` (fake sign-in only — no register, no onboarding; `provider/` guards as the Template) · `layout` (the "Hôm nay" shell — a Building scope **tabs row** under the header, ≤ 6 Toà nhà else a `Select`, `null` = every Toà nhà; `sidebar` 14 areas in four groups by chuỗi việc — Tháng này · Người & phòng · Sổ sách · Hệ thống — + `nav-user`; mobile (`< md`): a **4-item bottom nav** — Hôm nay · Phòng · Người thuê · Thu tiền (same `/invoices` route, filtered + sorted by hạn) — plus a "Thêm" Sheet with an account row and its own "Đăng xuất", the header down to title + search + bell) · `dashboard` (`/`, "Hôm nay": three KPI scoped to the Building scope — còn phải thu, Hợp đồng sắp hết hạn, tiến độ Chỉ số của Kỳ — a Việc cần làm queue that gộp Hoá đơn quá hạn by Toà nhà and sorts every entry by hạn rather than by source, and two "Tháng này"/"Vừa xong" number cards replacing the old donut + chart) · `cycles` (`/cycles/:month`, ADR-0013 — one Toà nhà, one Kỳ: chỉ số cũ, chỉ số mới pre-filled from a saved Nháp, tiêu thụ, tiền phòng/điện/nước/tổng computed on the spot from the Toà nhà's Bảng giá, "Lưu nháp chỉ số" and "Lập *n* hoá đơn" gated on the kỳ's ngày chốt — replaces the old Đợt hoá đơn + Nhập chỉ số screens) · `buildings` · `rooms` (floor grid, no pagination, grouped by Toà nhà too at scope null; a real guarded delete + edit on its detail screen) · `tenants` + the two Khai báo lưu trú obligations (Thông báo lưu trú now asks for a real mã hồ sơ + ngày gửi through a sheet; Đăng ký tạm trú's trạng thái is derived from its hạn) · `contracts` + Cọc (a **2-step** wizard pre-filled from Phòng/Toà nhà with a live tóm tắt, Gia hạn history, Thanh lý settling the deposit, and a header lifecycle stepper instead of a 520px sidebar card) · `invoices` (line items, Thanh toán pre-filled with phần còn lại, VietQR with its own "Đã nhận", Gửi nhắc log, CSV/print) · `utilities` (Chỉ số điện nước, Bảng giá-priced, anomaly-gated — list + detail only, reached from a Phòng's Chỉ số tab or the Kỳ screen, no longer its own sidebar row) · `supplier-bills` · `expenses` · `reconciliation` (derived from Hoá đơn + Hoá đơn NCC + Chi phí, never its own Mock) · `reports` (scoped chart or cross-Toà-nhà comparison, now also home to the revenue chart moved out of `dashboard`) · `communications` (templates + a real send-log) · `settings` (hồ sơ + "Khôi phục dữ liệu mẫu", per-Toà-nhà settings living on its own detail tab instead). Round 3 removed the `tasks` slice and its `/tasks` route outright — Việc cần làm has no screen of its own any more, only Hôm nay's queue and the bell
│   │   ├── src/stores/         ← `use-auth-store.ts` (token + `user` for nav-user) · `use-building-store.ts` (**Building scope**, `persist`, `null` = every Toà nhà)
│   │   ├── `~/hooks/use-delete-entity.ts` ← `useDeleteEntity({ mutation, id, label, entity?, successMessage, redirectTo })` → `{ open, onOpen, dialogProps }` — the confirm dialog's "không thể hoàn tác" copy, the mutate → toast → `navigate(redirectTo, { replace: true })` chain, written once for the six detail screens that delete (Phòng, Toà nhà, Chi phí, Hoá đơn NCC, Hợp đồng nháp, Hoá đơn — the last through a real no-op `useDeleteInvoice`, since a Hoá đơn has no delete state to remove)
│   │   └── `~/libs/http-client.ts` exports only `httpClient` — no service class until `be-motel` has a contract. `~/libs/mock-world.ts`'s `readWorld` (ADR-0015 §1) is the **one** function outside a mutation's own `mutationFn` allowed to touch `~/constants/mock` — it binds `~/utils/world.ts`'s pure `buildWorld` to the live Mock, and every derived `queryFn` reads through it rather than ghép tay mảng Mock; `test/hooks/api/world-write-seam.test.ts` scans every `~/hooks/api/*` file's text to hold both halves of the seam (the read half, and a write half that invalidates every query from one `MutationCache.onSuccess` in `~/libs/query-client.ts` rather than a hook's own list of keys). World also carries the derived `RoomView`/`TenantView`/`BuildingView` join (ADR-0015 §2) that replaced three families of stored-but-stale fields. `~/hooks/api/setting.ts`'s `useResetMockData` calls each entity's own `resetMock<Entity>` (`trackMockReset`, in-memory only — a reload already resets everything) so "Khôi phục dữ liệu mẫu" restores the lot in one mutation. The Mock lives in **`~/constants/mock/<entity>.ts`**, written to the `be-motel` contract shapes (ADR-0012) rather than free-hand, not inside the slice the spec named: `~/hooks/api` serves it and a hook may not import `~/features`
│   ├── chat/                    ← `@monorepo/chat` — a real-time Messenger-style app, ported 1:1 from `chat-socket-fe` (Rsbuild + STOMP, `D:\Personal\chat\chat-socket-fe`) onto backend `chat-socket` (Spring Boot, REST + STOMP on one port). Phase 1 (spec [#195](https://github.com/qtuan02/monorepo/issues/195)) got it running with no redesign; spec [#232](https://github.com/qtuan02/monorepo/issues/232) then redesigned it into shape **"Islands"** (brief `docs/design/chat-redesign.md` §10, ADR-0016, glossary `apps/chat/CONTEXT.md` — Islands · Island · Rail · Bottom nav + phase-1 terms: Session, Health gate, Presence, Conversation, Draft conversation, Friend request): every region — Rail, conversation list, message pane, Details, Bottom nav — sits on an `Island` (`bg-card/75`, 22px radius, no `backdrop-filter`) floating on a static three-stop gradient the app paints at `body`; teal (`--primary`) marks *what to do* (New, Send, the visitor's own gradient bubble), near-black `--foreground` marks *where you are* (Rail active item, the selected chip, the "N new messages" button). `≥md` shows `NavRail`, `<md` a 3-item `BottomNav` (Chats · Friends · Me) hidden inside a Conversation screen; light theme plus a Light/Dark/System toggle ship together (context + one `localStorage` key + a `.dark` class, no store — `apps/documents`'s shape), and `.dark` is a genuine dark Islands theme (darker islands on a darker gradient), not a leftover half-finished palette. Runtime **Vite** (cloned from `_template_vite`), dev 3007 / E2E 3107 in its `ports.env`. Two env keys of its own — `PUBLIC_CHAT_API_BASE_URL` (the `chat-socket` origin; the service singleton appends `/api`) and `PUBLIC_CHAT_SOCKET_URL` (the STOMP-over-WebSocket URL) — via `baseEnvSchema.extend()`. Refresh-and-retry on a 401/403 is solved once in `@monorepo/api` rather than a second HTTP layer — [ADR-0014](../../docs/adr/0014-api-refresh-token-opt-in-http-client.md), `createHttpClient`'s opt-in `withCredentials`/`onAuthError`. `src/globals.css` keeps the source's own teal palette as an app-layer override (`--primary`/`--ring`/`--sidebar-primary`/tinted sidebar, `oklch()` kept rather than converted to hex) plus a token this app introduces, `--online`/`--color-online`, for the Messenger-style presence dot, and the two Islands gradient stops (`--islands-gradient-*`) declared inside `@layer base` rather than unlayered, since nothing in `theme.css` sets a literal value for either — same unlayered-`:root`/`.dark` override shape as `portfolio` (ADR-0008), `documents` (ADR-0009) and `smart-rental` (ADR-0011) for the rest of the palette. Deployed on Vercel like `smart-rental`. Full shape after #232 — `apps/chat/README.md` § Hình dạng Islands. **i18n arrived 2026-09-20** — the i18next Flavor (`~/libs/i18n.ts`, cookie `chat_lang`, `chat.*` namespace, `LanguageToggleButton` beside the theme toggle on the Rail); the Rail/Bottom nav/toggles are translated, the rest of the copy is still hardcoded English pending a sweep. Timestamps: `chat-socket` returns UTC, the FE renders local through `~/utils/date.ts` (`toLocal`) and sends UTC ISO through `toApiTimestamp`. **Island fallback (spec #251/#252, 2026-09-20)** — each Island with a query of its own sits behind its own `react-error-boundary` (`~/components/exception/island-boundary.tsx` + `island-fallback.tsx`), scoped to that Island's own query key(s) with Retry doing `resetQueries` rather than a full reload; every boundary's catch is logged once from `createRoot(rootEl, { onCaughtError, onUncaughtError })` in `src/index.tsx`. **Contract cutover (spec #253, 2026-09-20→21)** moved the whole data layer onto `chat-socket`'s new shapes and shipped four features the prior contract had no room for: Attachment (one file per message, `IMAGE` renders inline, `FILE` a download card), Sửa/Xoá tin (edit — `TEXT` only — and delete a visitor's own message, via a bubble's own ⋯ menu), Typing (a composer-line "X is typing…", throttled send), and changing password on `/profile`. **What it deliberately does not have**, recorded in its README: no `home` slice, no `~/services/` (every backend call goes through a service class in `@monorepo/api`), no optimistic send/"Failed · Retry", no message tombstone or image lightbox, no typing indicator in the conversation list, no server-side conversation search (out of scope since #232)
│   └── storybook/               ← `@monorepo/storybook` — previews `@monorepo/ui` (Storybook 10.6 + `@storybook/react-vite`, `addon-docs` only), port 6006 — its one port literal, the `-p 6006` in `package.json`; deliberately outside the 3000+n / 3100+n bands, and it has no E2E server, so it declares no `ports.env`. Its own README is the source of truth for the story conventions — the stage panel + `parameters.stage.width` scale, the self-written theme toolbar (no `@storybook/addon-themes`), and the Northwind fixtures every story's copy draws from; read it before touching `.storybook/preview.tsx` or a story file
│       ├── .storybook/main.ts · preview.tsx
│       ├── src/introduction.stories.tsx ← top-level `title: "Introduction"`, pinned first in the sidebar by `options.storySort`
│       ├── src/support/        ← Northwind fixtures — `people.ts` · `projects.ts` · `invoices.ts` · `notifications.ts`, one named export per entity, no barrel
│       ├── src/stories/        ← one `*.stories.tsx` per primitive + one `use<Name>.stories.tsx` per hook
│       ├── test/               ← `stories.test.tsx` (renders every story via `composeStories`) · `story-conventions.test.tsx` (the naming/args/width convention) · `preview-decorators.test.tsx` (stage width, theme sync, fullscreen) · `form-stories.test.tsx` · `date-picker-stories.test.tsx`
│       └── postcss.config.mjs · Dockerfile · nginx.conf
├── packages/                    ← ten workspaces: eight source packages plus the two **Publish shells**. All eight sources are `private: true`, source-only, subpath-only `exports` into `src/` — every app in this repo imports their `.ts`/`.tsx` directly, with no build step in between. Six of them (`api`, `dayjs`, `env`, `i18n`, `sentry`, `types`) have no `build` task and no `dist/` at all, which is decision 3 unchanged; `ui` and `hook` read exactly the same way inward, but each additionally carries a `build` task (rslib, bundleless — per-file ESM + `.d.ts`) whose output lands in its shell rather than in itself (ADR-0004). Six of them carry their own Vitest runner and a `test/` tree mirroring `src/` — `api`, `dayjs`, `env`, `ui` on the node environment, `hook` and `i18n` on jsdom; `sentry` and `types` have no test script
...
│   ├── hook/                   ← `@monorepo/hook` — 18 hooks: 17 **Derived** from [hooks-ts](https://github.com/michal-worwag/hooks-ts) 0.12.0 (MIT © 2024 Michał Worwąg) plus `use-is-mobile` (Own). Every Derived file opens with a `Derived from hooks-ts <File>.ts @ <SHA> (hooks-ts@0.12.0), MIT © 2024 Michał Worwąg — see LICENSE-hooks-ts` header plus a `patched:` line naming what the repo's rules/SSR forced it to change; `LICENSE-hooks-ts` ships verbatim in both Publish shells that still distribute this code. First package in `packages/` with its own Vitest runner (jsdom) — 15 upstream tests carried over plus one per patch, and `derived-header.test.ts` reads every file as text so a missing header or SHA fails the Gate. See ADR-0010 and the **Derived** term in `CONTEXT.md`. Exports `./*`
...
│   ├── ui/                     ← `@monorepo/ui` — 63 shadcn primitives in the `base-vega` style, on Base UI. Exports `./components/*` and `./utils/*` only. Internally uses Node subpath imports (`#components/*`, `#utils/cn`, `#hooks/*`); `scripts/guard-no-local-hooks.ts` keeps `#hooks/*` an empty landing pad so a generic hook lands in `@monorepo/hook`. Its runner covers the framework-free utils only — the primitives themselves are covered by `apps/storybook`. `scripts/build.ts` (rslib + a CSS step) fills `ui-public/dist`, inlining the two hooks `sidebar` needs (`use-is-mobile` + the `use-media-query` it imports) into `dist/internal/` so no `@monorepo/*` or `#…` specifier survives into the output
│   ├── ui-public/              ← the **Publish shell** for `@monorepo/ui`, published to npm as **`@fe-monorepo/ui`**. It holds a hand-written `package.json` — literal dependency ranges, never `catalog:` and never `workspace:`, because npm can resolve neither — plus a README written for a consumer rather than for this repo; its `dist/` is written by `@monorepo/ui`'s `build` and is **gitignored**. `dist/internal/` additionally vendors the three files `sidebar` needs and nothing more — `use-is-mobile.js`, the `use-media-query.js` it imports, and `LICENSE-hooks-ts` for the latter's hooks-ts attribution (ADR-0010) — so this shell depends on no sibling package. Publishes subpaths only (`./components/*`, `./utils/*`) plus the one CSS entry `./globals.css`
...
├── docs/
│   ├── adr/                    ← ADR-0001 (the legacy apps held outside the workspace, since migrated and deleted) · ADR-0002 (one i18n package, many Flavors, ICU messages) · ADR-0003 (env two Flavors, native prefixes) · ADR-0004 (npm publish through a Publish shell) · ADR-0005 (the React Router framework-mode Runtime, built from primary docs rather than copied) · ADR-0006 (the third env Flavor, self-contained rather than importing the Vite one) · ADR-0007 (SSR auth — a signed cookie session guarded by route middleware) · ADR-0008 (`apps/portfolio` wears a neubrutalist shape and overrides the shared neutrals at the app layer) · ADR-0009 (`apps/documents` wears the "Prism" shape — no sidebar, glass panels on an aurora backdrop — and overrides the whole shared palette at the app layer; the second app to override, for its own reason: a docs site for public npm packages does not wear an internal EMR brand) · ADR-0010 (`@monorepo/hook` is a Derived copy of `hooks-ts`, with attribution, rather than a free-hand rewrite or an npm dependency) · ADR-0011 (`apps/smart-rental` wears the "Hôm nay" shape — Building scope as the shell's own tabs, a task-queue landing screen — and overrides the shared accent at the app layer) · ADR-0012 (`apps/smart-rental`'s derived state is computed, never stored, and its Mock is written to `be-motel`'s own contract) · ADR-0013 (`apps/smart-rental`'s Kỳ is the unit of the monthly chốt-chỉ-số → lập-Đợt → thu-tiền flow, hạn thu is the following month's Ngày thu) · ADR-0014 (refresh-token is an opt-in option of `createHttpClient` in `@monorepo/api`, not a second axios instance per app) · ADR-0015 (`apps/smart-rental` has one read seam, World, and one write seam, invalidate-all after every mutation)
│   ├── design/                 ← the **design step**'s briefs (§7a), one per redesign round — `portfolio-rebuild.md` (v1, spec #103), `portfolio-redesign-v2.md` (v2, spec #113) and `documents-redesign.md` (`apps/documents`, round 3 — direction D "Prism", spec #128). A round's static HTML mockups sit beside it in `<brief-name>/mockup-*.html` (committed, Biome-ignored). Each is the record *at the moment of deciding*, headed with the spec that implemented it; the shape of the app afterwards is its README and §1 here, never these files
...
| A generic React hook (debounce, media query…) | `packages/hook/src/use-<name>.ts` — if [hooks-ts](https://github.com/michal-worwag/hooks-ts) has it, copy it in as **Derived** (source header + `LICENSE-hooks-ts`, patched only where a repo rule or SSR forces it) rather than writing it from scratch; write it as **Own** only when no upstream hook covers it. Not `packages/ui/src/hooks/` — the `#hooks/*` alias exists only so the shadcn CLI will run, and the directory must not exist; `guard:no-local-hooks` fails `ui-add` if the CLI scaffolds one                        | ADR-0010, `architecture-feature-boundaries.md`, `quality-imports.md`               |
...
| An env variable                               | Add it (with a dev value) to the root `.env.example` under the right prefix group, then extend the schema inside that app's `src/env.ts` — `baseEnvSchema.extend()` for a Vite app, a `server`/`client` entry plus a literal `clientRuntimeEnv` read for a Next app, and for a React Router app a `server`/`client` entry plus a line in the **full** `runtimeEnv` map — env-core reads only that object and never falls back to `process.env` per key, so a client key is a literal `import.meta.env.PUBLIC_*` read and a server key is guarded with `typeof process === "undefined"`. **Name it for who owns it.** A value every app reads keeps the plain key (`PUBLIC_BASE_DOMAIN_API`, `NEXT_PUBLIC_APP_ENV`); a value only **one** app reads carries that app's name — `NEXT_PUBLIC_<APP>_…` in the Next Runtime, `PUBLIC_<APP>_…` in the Vite Runtime, `<APP>_…` for a server-only secret — and the three Templates are apps too: `TEMPLATE_VITE` / `TEMPLATE_NEXT` / `TEMPLATE_REACTROUTER`. There is one `.env` at the root for the whole workspace, so two apps that reuse a key are not sharing a default, they are each building with the other's value. `apps/_template_next` declares `NEXT_PUBLIC_TEMPLATE_NEXT_SENTRY_DSN` and `apps/portfolio` its own `NEXT_PUBLIC_PORTFOLIO_SENTRY_DSN`; `apps/documents` declares `PUBLIC_DOCUMENTS_STORYBOOK_URL`; `apps/mcp` declares the server-secret form, `MCP_OPENWEATHERMAP_API_KEY`; `apps/_template_reactrouter` declares the same form, `TEMPLATE_REACTROUTER_SESSION_SECRET` (unprefixed, required), and reuses the shared `PUBLIC_APP_ENV` / `PUBLIC_BASE_DOMAIN` / `PUBLIC_BASE_DOMAIN_API` client keys. | ADR-0003, ADR-0006, `next-env-t3.md`, `reactrouter-i18n-env.md`, `packages/env/README.md` |
...
bun run dev:mcp           # the MCP server (Next)         — http://localhost:3004/api/mcp
...
bun run dev:chat                  # the chat app (Vite)           — http://localhost:3007
...
- **Đầu ra của bước design** là một tài liệu tiếng Việt cho grill: ràng buộc UX đã chọn (mỗi cái trỏ `No`
  ...
  `~/components`, token delta so với `theme.css`, state list, copy cần dịch, câu hỏi mở. Brief lưu ở
  `docs/design/<app>-<round>.md`. **Mockup HTML tĩnh đi kèm brief và commit trong repo** (quyết định
  của chủ repo 2026-09-16): `docs/design/<app>-<round>/mockup-*.html`, một file tự chứa (inline CSS,
  không script, không font ngoài), mỗi hướng một frame để chọn bằng mắt trước grill; `biome.json` bỏ
  qua `docs/design/**/*.html` vì đó là bản ghi, không phải source. Mockup là bản ghi *tại thời điểm
  chọn* như chính brief — không phải nguồn sự thật cho code; cần nhìn bố cục trên route thật thì
  `prototype/UI.md`, sau khi đã có ticket.
...
This project is indexed by GitNexus as **monorepo** (10232 symbols, 20494 relationships, 274 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
