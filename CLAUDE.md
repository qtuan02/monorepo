...
│   │   ├── src/globals.css     ← the three things a CV needs that the shared theme does not ship. **A palette of its own — twelve colour tokens and one length**, written **unlayered** because `theme.css` arrives through a bare `@import` and an unlayered declaration beats one in any `@layer`: v1's **seven-token indigo accent** (hue 277) overriding the EMR palette's teal; the **two neutrals** v2 takes over, `--foreground` and `--border`, pushed to near-black in light and near-white in dark (`--muted-foreground` stays the theme's, deliberately); the **highlight pair**, one yellow (hue 91–92) for exactly two roles — the Email action's fill and the VDA 2025 award badge — with a duller value in dark and its focus ring drawn inset in `--highlight-foreground` because no yellow clears 3:1 against the indigo `--ring` there; `--hard-shadow`, the colour of the solid 4px shadow (`shadow-hard` in `@theme inline`), a token so light and dark flip it with the border; and `--radius: 0px` **with a unit**, which squares every `rounded-*` primitive from `theme.css`'s `calc()` derivations in one line. The dark theme is the light one **inverted, not dimmed**. `test/globals.test.ts` pins the token set, hues, poles, every contrast ratio in both themes, the radius and the cascade placement — extend it on purpose, never loosen it. Then the `prefers-reduced-motion` branch, which switches off by name the one animation CSS still owns — the theme wipe — now that no section fades in, the hero no longer waves, the dock no longer magnifies and nothing on the page starts at `opacity: 0`; and `@media print`, because this page **is** the CV and the hero's "download" is the browser's own print dialog — it keeps only what a utility cannot say (the dark→light palette swap, forcing folded rows open, printing an external link's href), while how a block prints lives beside its screen classes in `standard-block.tsx`
