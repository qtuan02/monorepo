...
│   ├── smart-rental/            ← `@monorepo/smart-rental` — the landlord's **Portal** for managing rented rooms. Phase 1 (spec #127) ported the prototype `fe-motel-rsbuild` onto Mock; phase 2 (spec #153, glossary `apps/smart-rental/CONTEXT.md` — Portal, Mock, Toà nhà, Building scope, Chỉ số điện nước, Đợt hoá đơn, Việc cần làm…) redesigned it into **"Hôm nay"** — shipped. Runtime **Vite** (cloned from `_template_vite`), dev 3006 / E2E 3106 in its `ports.env`, no env key of its own. Deployed on **Vercel** like `documents` — `vercel.json` (install/build via `npx --yes bun@1.4.0` from the repo root, SPA rewrite), the three base `PUBLIC_*` keys from the dashboard; the Template Dockerfile stays for the CI `docker` job. The prototype repo `fe-motel-rsbuild` is archived. **What it deliberately does not have**, recorded in its README and not drift to synchronise back: no i18n (hardcoded Vietnamese copy, `setDayjsLocale("vi")` once in `src/index.tsx`, `@monorepo/i18n` off its deps), no dark-mode **toggle** (the token pair is written, unused — see below), no `home` slice (`/` is "Hôm nay"), no sign-up/onboarding (removed in phase 2 — a fake sign-in is the whole auth surface), and no manual Task entry (Việc cần làm is entirely derived, read-only at `/tasks`; a maintenance-ticket slice waits on a real `maintenance-service`)
│   │   ├── src/globals.css     ← [ADR-0011](../../docs/adr/0011-smart-rental-ledger-palette-override.md): an unlayered `:root`/`.dark` block overriding the shared theme at the app layer, the same move `portfolio` (ADR-0008) and `documents` (ADR-0009) make — navy `--primary`/`--ring`/`--sidebar-primary`, `--radius: 0.375rem`, IBM Plex Sans via `@fontsource-variable`. `.dark` ships fully written but is never toggled to; `--success`/`--warning`/`--info`/`--destructive` and every `--chart-*`/`--sidebar-*` stay the theme's own, so a status colour never changes meaning between apps. `test/globals.test.ts` (contrast + token-parity, copied from `portfolio`) is the gate
...
│   │   ├── src/features/       ← `auth` (fake sign-in only — no register, no onboarding; `provider/` guards as the Template) · `layout` (the "Hôm nay" shell — a Building scope **tabs row** under the header, ≤ 6 Toà nhà else a `Select`, `null` = every Toà nhà; `sidebar` 15 areas in three groups + `nav-user`; mobile (`< md`): a **5-item bottom nav** — Hôm nay · Phòng · Người thuê · Hoá đơn · Thêm — the header down to title + search + bell) · `dashboard` (`/`, "Hôm nay": KPI strip scoped to the Building scope, and the Việc cần làm queue derived from five sources — overdue Hoá đơn, Hợp đồng sắp hết hạn, Chỉ số bất thường, Thông báo lưu trú chưa gửi, kỳ chưa lập Đợt — each row a real action) · `buildings` · `rooms` (floor grid, no pagination, a real guarded delete + edit on its detail screen) · `tenants` + the two Khai báo lưu trú obligations · `contracts` + Cọc (a four-step wizard, Gia hạn history, Thanh lý settling the deposit) · `invoices` (line items, Thanh toán, VietQR, Gửi nhắc log, CSV/print, Đợt hoá đơn gated on Chỉ số) · `utilities` (Chỉ số điện nước, Bảng giá-priced, anomaly-gated) · `supplier-bills` · `expenses` · `reconciliation` (derived from Hoá đơn + Hoá đơn NCC + Chi phí, never its own Mock) · `reports` (scoped chart or cross-Toà-nhà comparison) · `communications` (templates + a real send-log) · `tasks` (`/tasks` — Hôm nay's own queue, read-only, filtered on the URL) · `settings` (hồ sơ + "Khôi phục dữ liệu mẫu", per-Toà-nhà settings living on its own detail tab instead)
...
This project is indexed by GitNexus as **monorepo** (7498 symbols, 14644 relationships, 195 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
