...
**Status:** done (smoke test chạy 2026-09-04 trên nhánh `feat/upgrade`, chưa commit; ô CI chờ push — xem "Còn treo")
...
- [~] Gate xanh local và trên CI — **local xanh (4/4, exit 0), CI chưa chứng minh:** `feat/upgrade` chưa có trên remote (`git branch -r` chỉ có `dev`/`enhance`/`feat/google`/`main`) và toàn bộ `turbo/` vẫn untracked, nên chưa workflow run nào nhìn thấy generator. Cùng lý do với ô CI của ticket 01
...
- **Ô CI chưa tick** — cùng lý do ticket 01: `feat/upgrade` chưa push, `turbo/` còn untracked.
...
- **Job `e2e` trên CI không phủ app sinh ra.** `.github/workflows/ci.yml:150-165` liệt đích danh hai Template app thành hai step và upload đúng đường dẫn report của chúng; generator không đụng workflow. Bộ lọc `changes` khớp `^apps/` nên job **vẫn chạy** khi diff chạm app mới — chỉ là chạy hai template, đọc như một độ phủ không có thật. Fix là gọi fan-out (`bun run e2e` + truyền `PLAYWRIGHT_BROWSERS_PATH` qua Turbo) và đổi artifact path sang `apps/*/playwright-report/` + `apps/*/junit.xml`, **nhưng chỉ an toàn sau khi port đã tách** — fan-out song song trên cùng port 3000 còn tệ hơn khoảng trống hiện tại. Thuộc **ticket 01/08**.
