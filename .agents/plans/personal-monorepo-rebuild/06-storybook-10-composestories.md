...
**Status:** done (implement xong từ trước; xác minh lại 2026-09-04 khi chạy Gate, ô kiểm tay chuyển sang ticket 12 — xem "Còn treo")
...
- [ ] Dockerfile + nginx.conf cho storybook-static copy từ reference; `docker build` chạy được (kiểm tay, ghi kết quả) — **file có (`Dockerfile`, `nginx.conf`), nhưng chưa có kết quả `docker build` nào được ghi**; chuyển sang ticket 12
- [ ] Checklist kiểm tay: Slider ngang/dọc, ScrollArea, Tabs vertical, Separator hiển thị đúng; Dialog/Popover/Tooltip không bị z-index che — **chưa có kết quả nào được ghi**; chuyển sang ticket 12
- [~] Root script `dev:storybook`; Gate xanh local và trên CI — **script có** (`"dev:storybook": "turbo watch dev -F @monorepo/storybook..."`), **Gate local xanh**, **CI chưa chứng minh** (`feat/upgrade` chưa push, cùng lý do ticket 01)
...
- **Hai ô kiểm tay** (docker build của image storybook; checklist orientation Slider/ScrollArea/Tabs/Separator + z-index Dialog/Popover/Tooltip) **để nguyên chưa tick** và chuyển sang **ticket 12**, là ticket sở hữu lượt kiểm tay cuối. Không tick theo suy đoán: đúng cái checklist orientation này tồn tại vì jsdom **không** bắt được nó — hai `@custom-variant data-horizontal/data-vertical` trong `tooling/tailwind/globals.css` mà mất đi thì Tailwind sinh ra *không CSS và không lỗi*, và 148 test vẫn xanh.

- **Ô CI** — cùng lý do ticket 01: nhánh `feat/upgrade` chưa push.
