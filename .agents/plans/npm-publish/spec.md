...
24. As a developer, I want git tag `@fe-monorepo/ui@2.0.0` / `@fe-monorepo/hook@2.0.0` được push khi publish, so that mỗi version trên npm trỏ về một commit.
...
38. As a developer, I want lần publish `2.0.0` đầu tiên đi qua đúng quy trình changeset (một changeset `major` cho mỗi shell với note "Base UI, 63 primitive / 5 hook, API mới hoàn toàn"), so that quy trình được chứng minh ngay lần đầu chứ không phải một `npm publish` tay rồi mới bật workflow.
...
- **Verify từng ticket:** `bun run check && bun run typecheck && bun run test && bun run build`, rồi `bun run publish:smoke`; `changeset status` để thấy release plan; lần release đầu quan sát workflow `release` mở PR rồi publish, kiểm `npm view @fe-monorepo/ui@2.0.0 --json` có `dist.attestations`.
...
- Thứ tự gợi ý cho `/to-tickets`: (1) `hook` → shell + build + smoke test; (2) `ui` → shell + build + inline hook + CSS + smoke test mở rộng; (3) `.changeset/` + `release.yml` + hai job CI + docs/rule; (4) `ready-for-human`: npm trusted publisher + GitHub setting, rồi lần release `2.0.0` đầu tiên và ghi kết quả.
